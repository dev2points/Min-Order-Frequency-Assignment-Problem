...
RESULT_DIR=results/no_preprocessing/dse_incsc
mkdir -p "$RESULT_DIR"
...
for ds in "${DATASETS[@]}"; do
  ./runlim -r "$TO" -s "$MO" python3 -u pairwise_no_preprocessing.py "$ds" nsc assumptions cadical195 2>&1 | tee "$RESULT_DIR/$ds.log"
done
